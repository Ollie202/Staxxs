...
export function render(): void {
  ...
  if (state.tab === "home") {
    ...
    const profileBtn = el("button", { type: "button", title: state.user ? "Open profile" : "Sign in or open profile", style: { width: "34px", height: "34px", borderRadius: "50%", border: "1px solid " + th.border, background: th.card, color: th.sub, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", padding: "0", fontSize: "13px", fontWeight: "800", fontFamily: "'DM Sans',sans-serif", flexShrink: "0" }, onClick: () => { setTab("profile"); window.scrollTo({ top: 0 }); render(); } });
    if (state.profile.avatar) {
      profileBtn.appendChild(el("img", { src: state.profile.avatar, alt: "Profile picture", style: { width: "100%", height: "100%", objectFit: "cover" } }));
    } else {
      const profileInitial = (state.profile.username || state.user?.email || "").trim()[0];
      if (profileInitial) profileBtn.textContent = profileInitial.toUpperCase();
      else profileBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>';
    }
    ...
    hdrR.append(profileBtn, shareBtn, thBtn, yrL, yrSpan, yrR);
  }
...
function renderProfile(th: Theme): HTMLElement {
  ...
  if (!state.showResetPanel) {
    ...
    const resetGrid = el("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: "10px", marginBottom: "12px", animation: "fadeIn .18s ease" } });
    resetGrid.append(
